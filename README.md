...
1. **پاسخ از پایگاه دانش (RAG):**  اگر سؤال درباره‌ی اطلاعات عمومی FAQ باشد، از یک پایگاه دانش کوچک (چند سند متنی) در **Chroma** (پایگاه‌داده‌ی برداری) بازیابی انجام میشود و پاسخ با کمک LLM ساخته میشود.
...
تصمیم اینکه کدام مسیر طی شود، توسط یک **گراف LangGraph** به‌صورت خودکار و بر اساس محتوای پیام گرفته میشود. مدل زبانی مورد استفاده از طریق **Groq** (رایگان، سازگار با API اوپن‌ای‌آی، بسیار سریع) فراخوانی میشود.
...
یک کلید دمو به‌صورت جداگانه در اختیار شما قرار داده شده است. در غیر این صورت، در [console.groq.com](https://console.groq.com) ثبت‌نام کنید و یک API Key بسازید.

### ۲. اجرا با Docker (کل پروژه با یک دستور)
